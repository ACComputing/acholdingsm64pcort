# Ultra Mario 3d 0.1 - SM64-colored Pygame 3D platformer.
# Entry point: python ###ultramario4k.py
import pygame
import math
import sys
import random
...
FPS = 60
N64_LOGIC_FPS = 30.0  # 60 FPS rendering, N64/SM64-style 30 Hz movement timing.
MAX_DT = 1.0 / 20.0   # Prevent huge physics jumps after stalls/window dragging.
FOV = 500
VIEW_DISTANCE = 6000
...
class Mario(Mesh):
    ...
    def update(self, floor_y=0, frame_scale=1.0):
        # frame_scale keeps the game speed stable while rendering at 60 FPS.
        # With N64_LOGIC_FPS=30, each 60 FPS visual frame advances half an N64 tick.
        self.dy -= GRAVITY * frame_scale
        self.y += self.dy * frame_scale
        if self.y < floor_y:
...
class Star(Mesh):
    def __init__(self, x, y, z, sid=0):
        ...
        self.add_cube(10,40,10,0,0,0,BUTTON_GOLD)
        self.add_cube(40,10,10,0,0,0,BUTTON_GOLD)
        self.add_cube(10,10,40,0,0,0,BUTTON_GOLD)
...
def build_bob_omb_battlefield():
    ...
    for bx,bz in [(200,-200),(-300,100),(400,200)]:
        ...
        m.add_cube(6,10,6, bx,22,bz, (207, 161, 129))  # fuse
...
def build_cool_cool_mountain():
    ...
    m.add_cube(40,56,4, -300,28,-338, (140, 79, 28))  # door
    m.add_cube(24,24,4, -260,48,-338, (170, 161, 172))  # window
    # Snowman
...
def main():
    ...
    pygame.display.set_caption("Ultra Mario 3d 0.1")
    ...
    def draw_sm64_sky(level_id):
        """SM64 PC port gradient sky, cached so 60 FPS spends work on gameplay/rendering."""
        sky = SM64_SKIES.get(level_id, ((80,144,248),(184,216,248),(160,200,240)))
        top, bot, fog = sky
        key = (level_id, top, bot)
        ...
        if cached is None:
            ...
            for y in range(HEIGHT):
                ...
                pygame.draw.line(sky_surface,(max(0,min(255,r)),max(0,min(255,g)),max(0,min(255,b))),(0,y),(WIDTH,y))
            sky_cache[key] = (sky_surface, fog)
        else:
            sky_surface, fog = cached
    ...
    while running:
        ...
        frame_scale = dt * N64_LOGIC_FPS
        if frame_scale <= 0:
            frame_scale = N64_LOGIC_FPS / FPS
        ...
        if current_state == STATE_MENU:
            ...
            menu_head.yaw += 0.02 * frame_scale
            ...
            ts = font_title.render("Ultra Mario 3d 0.1",True,WHITE)
            th = font_title.render("Ultra Mario 3d 0.1",True,RED)
            screen.blit(th,(WIDTH//2-ts.get_width()//2+3,ly+3))
            screen.blit(ts,(WIDTH//2-ts.get_width()//2,ly))
            sub = font_small.render("SM64 COLORS - ALL 26 LEVELS",True,YELLOW)
            ...
            if keys[pygame.K_LEFT]: cam_yaw -= KEY_TURN * frame_scale
            if keys[pygame.K_RIGHT]: cam_yaw += KEY_TURN * frame_scale
            if keys[pygame.K_UP]: cam_pitch = min(PITCH_MAX, cam_pitch + KEY_TURN*0.7 * frame_scale)
            if keys[pygame.K_DOWN]: cam_pitch = max(PITCH_MIN, cam_pitch - KEY_TURN*0.7 * frame_scale)
            ...
            drag = MOVE_DECEL ** frame_scale
            vel_x = (vel_x + accel_x * frame_scale) * drag
            vel_z = (vel_z + accel_z * frame_scale) * drag
